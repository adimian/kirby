...
def test_scheduler_queue_jobs(scheduler, single_job_description_json):
    scheduler.queue_job(single_job_description_json)
    assert scheduler.queue_daemon.next() == single_job_description_json


def test_scheduler_does_not_queue_twice_within_wakeup_period(
    scheduler, single_job_description_json
):
    date_1 = datetime.datetime(2000, 1, 1, 0, 0, 0)
    date_2 = datetime.datetime(2000, 1, 1, 0, 0, 20)

    scheduler.queue_job(single_job_description_json, now=date_1)
    with pytest.raises(CoolDownException):
        scheduler.queue_job(single_job_description_json, now=date_2)
